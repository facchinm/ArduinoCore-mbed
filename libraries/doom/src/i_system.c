...
static char *EscapeShellString(char *string)
{
    ...
    printf("[%s] ea_malloc %d\n", __func__, strlen(string) * 2 + 3);
    result = ea_malloc(strlen(string) * 2 + 3);
    r = result;
...
static int ZenityErrorBox(char *message)
{
    ...
    printf("[%s] ea_malloc %d\n", __func__, errorboxpath_size);

    errorboxpath = ea_malloc(errorboxpath_size);
    M_snprintf(errorboxpath, errorboxpath_size, "%s --error --text=%s",
               ZENITY_BINARY, escaped_message);

    result = system(errorboxpath);

    ea_free(errorboxpath);
    ea_free(escaped_message);
